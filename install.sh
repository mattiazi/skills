...
#  the version preinstalled on macOS. No xargs: safe with paths
#  containing spaces.)
ALL_SKILLS=()
while IFS= read -r skill_md; do
  ALL_SKILLS+=("$(basename "$(dirname "$skill_md")")")
done < <(find "$SKILL_PATH" -maxdepth 2 -type f -name "SKILL.md" | sort)
...
for skill in "${SKILLS_TO_INSTALL[@]}"; do
  ...
  if [ -e "$DEST" ] && [ "$FORCE" -ne 1 ]; then
    # Non-interactive run (e.g. curl | bash): don't prompt, skip instead.
    if [ ! -t 0 ]; then
      echo "  → '$skill' already exists, skipped (use --force to overwrite)"
      SKIPPED=$((SKIPPED + 1))
      continue
    fi
